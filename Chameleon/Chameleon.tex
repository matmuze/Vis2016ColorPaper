...
\begin{document}
	...
	All levels can be present; we can start at the compartment level and zoom in past the proteins, their domains and secondary structures, all the way to the amino acids and atoms. Visualizations of HIV are not only used for research purposes, but also to communicate its basic properties to a broader audience, as in Figure~\ref{fig:motivation}, which is what we are interested in.\\
	...
	For example, if we apply color mapping to proteins, then proteins are well conveyed, but secondary structures or atoms are not.
...
%	
	...
	However, this requires manually setting up the color palette for each level, which can be cumbersome when dealing with a multitude of large datasets.
	Our goal is to provide a dynamic coloring mechanism that automatically assigns and displays the most relevant information for any given zoom level, thereby fulfilling the requirements stated in the introduction: 
	...
	Finally, we indicate structures from lower hierarchies in the \textbf{luminance} channel to mimic illustrator techniques (Section \ref{sec:luminance}). 
	...
	In the highest hierarchy levels (protein and compartment), our task is to find colors for each individual protein type that allow users to discriminate the proteins, but also understand their hierarchical grouping into the compartments. 
	...
	As the user zooms in and explores the scene, large parts of the scene may not actually be visible in the current viewport, as is illustrated in the lower row of Figure~\ref{fig:coloroverview}.
	In this configuration the color palette becomes underexploited, and can improved to increase discriminability. 
	...
	In order to determine which protein groups occupy unnecessary space in the color palette, we extract the visibility information by analyzing the previously rendered frame.
	For each protein group we derive a visibility coefficient in order to scale their corresponding section on the hue circle. 
...
%	A protein type will also be set as visible if some elements are located close to the camera and occupy a significant number of pixels in the viewport, even when the overall percentage of visible elements is low.
	...
	Once we compute the coefficient of each protein group, we divide the hue circle proportionally to the group coefficient to determine the size of individual hue sections. \\
...
%	In the example of Figure \ref{fig:zoomLevels}, we use a luminance range of 20 to distinguish the sub-elements. 
	...
	\begin{enumerate}
		...
		\item the binding site of cyclophilin A, which is a loop on the surface of the capsid protein with several prline amino acids, and
	...
	We decided for a qualitative evaluation since there is no clear baseline for a comparative lab experiment. An comparison without secondary structure hue modulation can be seen in Figure~\ref{fig:comparison} (\textbf{TODO: show image without color coding for illustration! DONE: see figure}). 
	...
	\textbf{TODO: why did user 3 fail? Why was amino acid so hard to find? DONE: Look at 2 previous sentences}
	...
	\begin{figure}
		...
		\caption{Comparison between hue modulation for secondary structures (left) and no modulation (right)}
...
\begin{table}
	 ...
	 Several (\textbf{TODO: how many??}) also referred to the alpha helices by color. 
...
This is an indication that identification by structure is a stronger visual cue than identification by color alone -- yet, color can be used if no structural cues are available, as in our protein domain example. 
...
On this level, alpha helices and beta sheets were discriminated by color. 
...
%	If two proteins are one behind the other, then they can be hard to tell apart.
	...
	There are several points that can be addressed in the future. The most obvious is to overcome the limitations mentioned above. 
