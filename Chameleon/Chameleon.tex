...
\author[authors]
{Nicholas Waldin$^{1}$, Mathieu Le Muzic$^{1}$, Maneula Waldner$^{1}$, Eduard Gröller$^{1,2}$, David Goodsell$^{3}$, Autin Ludovic$^{3}$, Ivan Viola$^{1}$ 
%	and auth2$^{2}$
...
%	Assuming no spacing between compartments (i.e.\, $s=0$), this would result in only 32 distinct colors, which is far below the number of proteins in the HIV model. 
	...
	A protein type is visible, if at least one screen pixel is occupied by a protein of type $j$. 
...
%	\textbf{Essentially, all it does is determine if two groups overlap or not and to what degree. If they do no overlap, the force is equal to 0. The w takes the user specified minimum distance between wedges into account.}
	...
	Finally, the hue of a fragment is computed by performing a linear interpolation of the hue value of the two levels the user is between, as illustrated in Figure \ref{fig:zoom_continuum}.
...
%	Figure~\ref{fig:results_2} shows all the sequential steps we take to progressively reveal the protein properties as we zoom in.
	...
	\section{User Study}
	\label{sec:study}
	Since interactively explorable multi-scale visualizations of biology models have only become available very recently, there is no comparable approach how to represent biological structures across scales. 
	Dynamic visual discrimination, apart from geometric levels of detail \cite{parulek}, have not been studied so far in the biological field. We therefore conducted a qualitative user study to answer two research questions: 
	\begin{enumerate}
	...
	The task description for such users would have to be rater long and detailed, leading likely to issues with text comprehension or revealing too much information. 
...
\begin{table}
	 	...
	 	{\scriptsize \begin{tabular}{ l l l l l l }
	 		...
	 		Task \textbackslash User & 1 & 2 & 3 & 4 & 5 \\
	 		\midrule
	 		Pentamer       & \textbf{\color{cc}c} (100\%) 	& \textbf{\color{cc}c} (90\%) 	& \textbf{\color{ci}i} (70\%) 	& \textbf{\color{cc}c} (5) 		& \textbf{\color{cc}c} (5)\\
	 		C-terminal     & \textbf{\color{cc}c} (100\%) 	& \textbf{\color{cc}c} (70\%) 	& \textbf{\color{ci}i} (10\%) 		& \textbf{\color{cc}c} (4) 	& \textbf{\color{cc}c} (4) \\
	 		N-terminal     & \textbf{\color{cc}c} (100\%) 	& \textbf{\color{cc}c} (70\%) 	& \textbf{\color{ci}i} (10\%) 		& \textbf{\color{cc}c} (2) 	& \textbf{\color{cc}c} (4)\\
	 		$\alpha$-helix & \textbf{\color{cc}c} (guess) 	& \textbf{\color{cc}c} (50\%) 	& \textbf{\color{cn}n} 		& \textbf{\color{cc}c} (4) 	& \textbf{\color{cc}c} (2) \\
	 		Binding site   & \textbf{\color{cc}c} (1\%) 	& \textbf{\color{cc}c} (85\%) 	& \textbf{\color{cc}c} (90\%) 	& \textbf{\color{cc}c} (5) 		& \textbf{\color{cc}c} (4) \\
	 		Methionine      & \textbf{\color{cc}c} (1\%) 	& \textbf{\color{cp}p} (90\%) 	& \textbf{\color{cn}n} 		& \textbf{\color{cp}p (1)} 			& \textbf{\color{cn}n}  \\
	 		\bottomrule
	 	\end{tabular}}
...
In the questionnaire, most users indicated that they noticed this effect (see Figure \ref{fig:questionnaire}), but the confusion caused by these color changes were rated fairly low. 
...
%	This would allow the user easier inspection of proteins or protein structures by making it easier for him to rotate around or follow the surface of an object.
	...
	The first and second authors contributed equally.
		...
		Career Integration Grant through project PCIG13-GA-2013-618680.
