...
\author[Waldin et al.]
{Nicholas Waldin$^{1}$, Mathieu Le Muzic$^{1}$, Manuela Waldner$^{1}$, Eduard Gr\"oller$^{1,2}$, David Goodsell$^{3}$, Autin Ludovic$^{3}$, Ivan Viola$^{1}$ 
%	and auth2$^{2}$
...
\begin{document}
	...
	\teaser{
		...
		\begin{subfigure}[b]{0.195\linewidth}
			\centering
			\includegraphics[width=1\linewidth]{"Figures/teaser_newa"}
			\caption{\label{fig:teasera}}
		\end{subfigure}
		\begin{subfigure}[b]{0.195\linewidth}
			\centering
			\includegraphics[width=1\linewidth]{"Figures/teaser_newb"}
			\caption{\label{fig:teaserb}}
		\end{subfigure}
		\begin{subfigure}[b]{0.195\linewidth}
			\centering
			\includegraphics[width=1\linewidth]{"Figures/teaser_newc"}
			\caption{\label{fig:teaserc}}
		\end{subfigure}
		\begin{subfigure}[b]{0.195\linewidth}
			\centering
			\includegraphics[width=1\linewidth]{"Figures/teaser_newd"}
			\caption{\label{fig:teaserd}}
		\end{subfigure}
		\begin{subfigure}[b]{0.195\linewidth}
			\centering
			\includegraphics[width=1\linewidth]{"Figures/teaser_newe"}
			\caption{\label{fig:teasere}}
		\end{subfigure}
				\caption{Interactive multi-scale visualization of HIV using ``Chameleon'' for dynamic color coding of protein and atom properties: 
					Zooming from an overview of the entire virus (\subref{fig:teasera}) to the capsid (\subref{fig:teaserb}), the capsid interior (\subref{fig:teaserc}), a single integrase protein in the capsid (\subref{fig:teaserd}), and its atomic structure~(\subref{fig:teasere}). 
					Mind how protein colors become more differentiated as we zoom in (\subref{fig:teasera}-\subref{fig:teaserb}), and how protein domains (\subref{fig:teaserc}), secondary structures (\subref{fig:teaserd}), and individual atoms (\subref{fig:teasere}) are being progressively revealed through dynamic color adaptations.}
...
%			(e) The atom colors are progressively shown as we reach the final zoom level.}
	...
	For example, when looking at two neighboring compartments and zooming in, it may be hard to maintain an overview of which compartment certain proteins belong to. 
...
%	
	...
	\begin{figure}[t]
		...
		\label{fig:hueWheel}
	\end{figure}
	At the highest hierarchy level (compartment, as illustrated in Figure \ref{teaserImage}\subref{fig:teasera}), our task is to find colors for each individual protein type that allow users to discriminate the proteins, but also understand their hierarchical grouping into the compartments. 
...
%	Assuming no spacing between compartments (i.e.\, $s=0$), this would result in only 32 distinct colors, which is far below the number of proteins in the HIV model. 
	...
	\begin{figure*}[t]
		...
		\caption{Overview of Chameleon's dynamic and hierarchical hue selection scheme for (\textcolor{blue}{a}) an exemplary two-level hierarchy of four protein groups (A,B,C,D) and a total number of 20 different protein types: For the current viewport (\textcolor{blue}{b}), visibility information is extracted (\textcolor{blue}{c}) and the color palette is updated. Zooming further towards protein type 10 splits up the protein type hue to three different domain colors (x,y,z).}
	...
	As the user zooms in and explores the scene, large parts of the scene may not actually be visible in the current viewport, as is illustrated in the lower row of Figure~\ref{fig:coloroverview}(\textcolor{blue}{a}-\textcolor{blue}{c}).
...
%	\textbf{TODO: again, is this correct? a scaling factor proportional to the total number of protein types in a group multiplied by the visible protein types -- could this be just the ratio visible by total? But this would not fit to Figure 4. My guess would be that simply $m_i$ of the previous section is set, and the smaller $m_i$, the bigger the possible angular distance between the hues. Please clarify!}
	...
	As visualized in Figure \ref{fig:coloroverview}\textcolor{blue}{d}, the hue ring is divided proportionally to the protein groups' numbers of visible proteins. 
...
%	This means the user can control how distinct groups are from each other, if he so desires.}
	...
	\begin{figure}
		...
		\begin{subfigure}[b]{0.49\linewidth}
			\centering
			\includegraphics[width=0.85\linewidth]{"Figures/force-basedLayoutSplitA"}
			\caption{\label{fig:figforce1}}
		\end{subfigure}
		\begin{subfigure}[b]{0.49\linewidth}
			\centering
			\includegraphics[width=0.85\linewidth]{"Figures/force-basedLayoutSplitB"}
			\caption{\label{fig:figforce2}}
		\end{subfigure}
		\caption{Force-based hue assignment: Initial wedge placement (\subref{fig:figforce1}) and resulting force-based placement (\subref{fig:figforce2}). Red arrows indicate the default hues for the wedge centroids, and black arrows represent the forces. }
...
% Wedges are defined in the second paragraph of Section 4 (each wedge represents the color range of a compartment)
	...
	We start by positioning a first wedge on the circle, and then place the remaining wedges subsequently on the circle, as illustrated in Figure \ref{fig:force-basedlayout}\subref{fig:figforce1}.
	...
	This could lead to unnecessarily large hue shifts for some protein groups while navigating, as shown in Figure~\ref{fig:force-basedlayout}\subref{fig:figforce1}.
...
%				The hue is equal to a linear interpolation of the two neighbouring levels shown in the middle line, here compartment and protein. Luminance is equal to a linear interpoation of the next two levels, here protein and domain, visible on the bottom line. The interpolation is based on the ratio of the distance to the neighbouring levels, here 0.3 and 0.7. }
	...
	In our HIV showcase, we use pre-defined atom color codes for the highest zoom level (see Figure \ref{teaserImage}\subref{fig:teasere}), to support domain knowledge, such as CPK coloring for atoms. 
	...
	%The main different between this approach is that we do not prevent overlaps, because of limitations of orginal tree color approach ** Manu or Nik explain reasons here **.
		...
		\begin{figure}[t]
			...
			\begin{subfigure}[b]{0.49\linewidth}
				\centering
				\includegraphics[width=0.85\linewidth]{"Figures/desaturateda"}
				\caption{\label{fig:sata}}
			\end{subfigure}
			\begin{subfigure}[b]{0.49\linewidth}
				\centering
				\includegraphics[width=0.85\linewidth]{"Figures/desaturatedb"}
				\caption{\label{fig:satb}}
			\end{subfigure}
			\caption{Without (\subref{fig:sata}) and with (\subref{fig:satb}) desaturated peripheral colors.}
			\label{fig:results_3}
		\end{figure}
...
%	In the example of Figure \ref{fig:zoomLevels}, we use a luminance range of 20 to distinguish the sub-elements. 
	...
	\begin{figure}[t]
		...
		\begin{subfigure}[b]{0.49\linewidth}
			\centering
			\includegraphics[width=0.85\linewidth]{"Figures/2x2a"}
			\caption{\label{fig:fig2x2a}}
		\end{subfigure}
		\begin{subfigure}[b]{0.49\linewidth}
			\centering
			\includegraphics[width=0.85\linewidth]{"Figures/2x2b"}
			\caption{\label{fig:fig2x2b}}
		\end{subfigure}
		\begin{subfigure}[b]{0.49\linewidth}
			\centering
			\includegraphics[width=0.85\linewidth]{"Figures/2x2c"}
			\caption{\label{fig:fig2x2c}}
		\end{subfigure}
		\begin{subfigure}[b]{0.49\linewidth}
			\centering
			\includegraphics[width=0.85\linewidth]{"Figures/2x2d"}
			\caption{\label{fig:fig2x2d}}
		\end{subfigure}
...
\begin{table}
	 	...
	 	\centering
	 	\caption{Structure identification task: \textbf{\color{cc}c} identified correct item, \textbf{\color{cp}p} partially correctly identified item, \textbf{\color{ci}i} identified incorrect item, \textbf{\color{cn}n} nothing found. User-reported certainty shown on a scale of 1 (lowest) to 5 (highest).}
	 	{\scriptsize \begin{tabular}{ l l l l l l }
	 		\toprule
	 		Task \textbackslash User & 1 & 2 & 3 & 4 & 5 \\
	 		\midrule
