...
\teaser{
	...
	\caption{Mutli-scale visualization of the HIV virus in real-time, using cellVIEW~\cite{muzic2015cellview} for the rendering, together with the presented technique "Chameleon" for dynamic color coding of protein and atom properties.
		...
		(a) At the highest zoom level the orange protein located inside the capsid are hardly distinguishable, as surrounding compartments which contain more various protein types (green, and purple) are prioritized on the color palette. 
		(b) As we zoom-in on the capsid proteins, other compartments are become less visible, which allows the capsid protein ensemble to occupy more space in the color palette.
		(c) As we continue zooming we reveal the domain of the protein by shifting the hue value progressively, simultaneous we also shift the luminance to hit the presence of underlying structures in the next levels.
		(d) The secondary structures are differentiated with the hue, and the atom types by luminance, analogously to the previous levels.
