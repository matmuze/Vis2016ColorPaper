...
However, this can not be done on just one level, as proteins, for example, can be viewed based on which compartment they belong to, but also on lower levels: a protein consists of different domains, which can, in turn consist of secondary structures such as $\alpha$-helices and $\beta$-sheets. 
These structures can again be broken down into amino acids, which consist of a few atoms. 
...
Furthermore, the kind of information which should be displayed may conflict with the visualization of nearby levels, simply by being of similar scale, such as amino acids that consist out of a handful of atoms, and the atoms themselves.

We propose a semantic zooming method for visualizing color-coded a virus multi-scale visualization based on a view-dependent hierarchical color-scheme. 
...
We divide the related work in two sections, in the first section we cover previous work on colors in visualization, and their perceptual impact on the viewer. 
...
To provide smooth and logical transitions across level we use a hierarchical color coding approach inspired from \cite.
...
Protein types of a given group are then given a unique hue value on corresponding arc, as shown in Figure \ref{noref}.
