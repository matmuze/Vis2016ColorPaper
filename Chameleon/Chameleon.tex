...
\end{figure}
	...
	\[
	\alpha_{compartment} = \frac{360}{\sum_{i} m_i - n \cdot s}. 
	\]
	...
	As we zoom further into the proteins, we begin to reveal the colors corresponding to the following levels (domains, secondary structures, atoms). 
	...
	This means that the wedge will grow linearly with the number of domains until it reaches the angle $\beta$. 
	As long as the angle of the wedge is below $\beta$, the angle between domains on the hue circle will be equal to $d$.
	When the number of domains times $d$ is greater than $\beta$, the angle between domains will begin to shrink, and will be eqal to $\beta / m$.
