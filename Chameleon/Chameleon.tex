...
%	\textbf{Essentially, all it does is determine if two groups overlap or not and to what degree. If they do no overlap, the force is equal to 0. The w takes the user specified minimum distance between wedges into account.}
	...
	The user determines the camera distances at which the levels are displayed for two reasons:
	...
	This means the object may have different sizes for the same camera distance in different visualizations.
...
We therefore conclude that Chameleon is a valuable extension to multi-scale molecular visualization, which causes no notable distraction, but allows for efficient and pleasing visual encoding of protein sub-structures. 
...
%	
	...
	While out method works for molecular data, it may not be generalizable to all types.
	Our data is limited to 5 different zoom levels. 
	No tests with an arbitrary number of levels has been conducted. 
	Another point is the switch in coloring type from a force-based layout (Section~\ref{sec:dynamic}) to hierarchical coloring (Section~\ref{sec:hierarchy}). 
	As such, the search for a completely generalized method is still open.
	Second is the visualization of different zoom levels at the same time. 
	We encoded lower levels through luminance modulation. 
	However, it might be possible to encode more information without running the risk of creating salt and pepper noise.
