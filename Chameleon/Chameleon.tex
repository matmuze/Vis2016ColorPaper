...
%	Based on the size of the protein and its location, we define a spherical region in 3D which constitute the focus region.
	Outside this region, we progressively decrease the chroma value as far as 0 as we zoom towards the focused protein, to ensure optimal color discriminability of the focus protein's colors with respect to all other colors in the scene, as can be seen in Figure~\ref{fig:results_3}. 
...
%	Figure~\ref{fig:results_2} shows all the sequential steps we take to progressively reveal the protein properties as we zoom in.
	...
	With the structure identification task, we could assess whether users with sufficient knowledge to understand molecular structures without additional text labels are able to identify the above structures using our system. 
