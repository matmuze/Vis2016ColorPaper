...
\textbf{MWA TODO: please help us writing a better overview}

Mesoscale datasets feature many relevant properties which are present at different scale levels.
The molecular properties that we provide in this work are, from large to small scale: Compartment (or group), protein type, protein domain, secondary structure and atom type.
...
Our goal is to provide a coloe scheme that would automatically display meaningful information at all zoom levels.
...
\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{"Figures/force-based layout"}
...
By default, each group is assigned a centroid hue value, which we refer to as default hue centroid.
As we move the camera and affect the visibility of the protein, we modulate the size of the groups on the hue ring.
Upon modulation of the group sizes we must also reposition the centroid group positions on the ring.
A naive non-iterative approach would be to start positioning a first group on the circle, and to place the remaining groups subsequently on the circle, given the size of each group.
...
We switch between the colors of the different levels based on the distance of the proteins to the camera and user defined distance values which delimit where the levels being and ends.
In between levels we perform linear color interpolation for smooth and logical transitions between the levels.

\subsection{Interpolating Between Levels}

The transition between the colors of the levels is performed in a post-processing operation in a dedicated shader program.
We addition to the instance unique ID texture mentioned in Section~\ref{sssec:extracting}, we also generate an additional off-line texture upon rendering, which contains the unique atom ID of each rendered atom.
Together, the unique atom and protein ID will allow us to access directly from the shader program, all the needed information concerning the rendered atoms, and which is priorly stored on the video memory.
We also use the depth information of the rendered 
...
We select the protein with the highest screen coverage in the central region the screen as focus element and label the remaining proteins as context. 
...
\begin{table}
	\caption{tbd, check position and text}
...
This is achieved while reaining the connection between structures on different levels in a logical manner via the hierarchical nature of the coloring.
Furthermore the coloring scheme is capable of showing structures on each level in a visually distinct manner. 
The user study performed with this software shows that users can find information in an HIV dataset on each level. 
